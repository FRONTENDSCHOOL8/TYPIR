      ...
      >
        {data.map((item) => (
          <motion.li key={item.id} layoutId={item.id + ''}>
            <div className='mb-[15px] flex flex-col justify-center items-center bg-red-200'>
              <img
                src={item.image}
                alt={item.alt}
                className={`w-[170px] bg-gray-100 rounded-2xl cursor-zoom-in`}
                style={{ height: `${item.height}px` }}
                onClick={() => onBoxClicked(item.id)}
              />

              <p className='w-[170px] text-center my-1 truncate'>{item.title}</p>
            </div>
          </motion.li>
        ))}
      </Masonry>
